namespace BARS.ClickHouse.Ado.Impl.Compress
{
    ...
    internal class ClickHouseCityHash
    {
        ...
        private static ulong Fetch64(byte[] src, int off)
        {
            return BitConverter.ToUInt64(src, off);
        }

        private static uint Fetch32(byte[] src, int off)
        {
        ...
        private ulong CityHash64WithSeed(byte[] s, int len, ulong seed)
        {
            return CityHash64WithSeeds(s, len, k2, seed);
        }

        private ulong CityHash64WithSeeds(byte[] s, int len,
        ...
        private static UInt128 CityHash128(byte[] value, int len)
        {
            ...
            if (value.Length >= 8)
            {
                var seed = new UInt128(Fetch64(value, 0) ^ ((ulong) value.Length * k0),
                                       Fetch64(value, value.Length - 8) ^ k1);
